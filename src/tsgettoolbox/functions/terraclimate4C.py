"""
terraclimate4C      global 1/24deg M:Monthly normals from Terraclimate
                    with 4deg C hotter climate.
"""
...
@tsutils.copy_doc(terraclimate4C_cli)
def terraclimate4C(
    lat: float,
    lon: float,
    variables=None,
    ...
    df = df.rename(columns=lambda x: f"{x}:+4C")

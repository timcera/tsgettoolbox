...
def test_pyspark_to_sparksql(ctx, people):
    ...
    assert list(map(set, odo(people, list))) == list(map(set, odo(sdf, list)))
...
def test_sparksql_to_frame(ctx):
    ...
    np.testing.assert_array_equal(
        result.sort_index(axis=1).values, df.sort_index(axis=1).values
    )
...
def test_schema_to_dshape():
    ...
    assert schema_to_dshape(ArrayType(IntegerType(), False)) == dshape("var * int32")

    assert schema_to_dshape(ArrayType(IntegerType(), True)) == dshape("var * ?int32")
...
def test_dshape_to_schema():
    ...
    assert dshape_to_schema("{name: string, amount: int32}") == StructType(
        [
            StructField("name", StringType(), False),
    ...
    assert dshape_to_schema("10 * {name: string, amount: ?int32}") == ArrayType(
        StructType(
            [
                StructField("name", StringType(), False),
                StructField("amount", IntegerType(), True),
...
def test_append_spark_df_to_json_lines(ctx):
    ...
    sdf = ctx.table("t")
    expected = (
        pd.concat([df, df]).sort("amount").reset_index(drop=True).sort_index(axis=1)
    )
...
@pytest.mark.xfail(
    raises=(py4j.protocol.Py4JJavaError, AnalysisException), reason="bug in sparksql"
)
def test_append(ctx):
...
def test_load_from_jsonlines(ctx):
    with tmpfile(".json") as fn:
        ...
        assert list(map(set, odo(result, list))) == list(map(set, odo(df, list)))
...
def test_load_from_dir_of_jsonlines(ctx):
    ...
    with jslines() as d:
        ...
        assert set(map(frozenset, odo(result, list))) == set(
            map(frozenset, odo(expected, list))
        )

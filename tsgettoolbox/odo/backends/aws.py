...
def get_s3_connection(
    ...
    **kwargs
    ...
    anon = not anon and aws_access_key_id is None and aws_secret_access_key is None
        ...
        **kwargs
        ...
        try:
            bucket = self.s3.get_bucket(
                self.bucket, validate=False, **filter_kwargs(self.s3.get_bucket, kwargs)
            )
        except boto.exception.S3ResponseError:
            bucket = self.s3.create_bucket(
                self.bucket, **filter_kwargs(self.s3.create_bucket, kwargs)
            )
        ...
        self.subtype.__init__(
            self, uri, *args, **filter_kwargs(self.subtype.__init__, kwargs)
        )
...
def sample_s3_line_delimited(data, length=8192):
    ...
    if data.object.exists():
        ...
        keys = sorted(
            data.object.bucket.list(prefix=data.object.key), key=attrgetter("key")
        )
        if not keys:
            # we didn't find anything with a prefix of data.object.key
            raise ValueError(
                "Object %r does not exist and no keys with a "
                "prefix of %r exist" % (data.object, data.object.key)
            )
        key = first(keys)
...
@drop.register((Temp(S3(CSV)), Temp(S3(JSON)), Temp(S3(JSONLines)), Temp(S3(TextFile))))
...
@append.register(
    (S3(JSONLines), Temp(S3(JSONLines))), (S3(JSONLines), Temp(S3(JSONLines)))
)
@append.register((S3(TextFile), Temp(S3(TextFile))), (S3(TextFile), Temp(S3(TextFile))))
def temp_s3_to_s3(a, b, **kwargs):
    a.object.bucket.copy_key(b.object.name, a.object.bucket.name, b.object.name)
    return a
...
@append.register(
    (S3(CSV), S3(JSON), S3(JSONLines), S3(TextFile)),
...
def start_multipart_upload_operation(s3_object, **kwargs):
    multipart_upload = s3_object.object.bucket.initiate_multipart_upload(
        s3_object.key, **kwargs
    )
...
def append_text_to_s3(s3_object, data, multipart=False, part_size=5 << 20, **kwargs):
    if multipart:
        filtered_kwargs = filter_kwargs(
            s3_object.object.bucket.initiate_multipart_upload, kwargs
        )
        with start_multipart_upload_operation(
            s3_object, **filtered_kwargs
        ) as multipart_upload:
            with open(data.path, "rb") as f:
                parts = enumerate(iter(curry(f.read, part_size), ""), start=1)
                for part_num, part in parts:
                    multipart_upload.upload_part_from_file(
                        BytesIO(part), part_num=part_num
                    )
        return s3_object
...
try:
    ...
    def s3_to_ssh(ssh, s3_object, url_timeout=600, **kwargs):
        ...
        if exit_status:
            raise ValueError(
                "Error code %d, message: %r" % (exit_status, stderr.read())
            )
        return ssh

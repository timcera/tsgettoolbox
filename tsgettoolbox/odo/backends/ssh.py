...
class _SSH(object):
    ...
    def __init__(self, *args, **kwargs):
        self.auth = keyfilter(keywords(paramiko.SSHClient.connect).__contains__, kwargs)
...
@sample.register((SSH(CSV), SSH(JSON), SSH(JSONLines)))
@contextmanager
...
@sample.register((SSH(Directory(CSV)), SSH(Directory(JSON)), SSH(Directory(JSONLines))))
@contextmanager
...
@discover.register(
    (SSH(Directory(CSV)), SSH(Directory(JSON)), SSH(Directory(JSONLines)))
)
...
for typ in [CSV, JSON, JSONLines, TextFile]:
    convert.register(Temp(SSH(typ)), (Temp(typ), typ))(file_to_temp_ssh_file(typ))

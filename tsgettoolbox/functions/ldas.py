...
_varmap = {
    ...
    "GRACEDADM_CLSM0125US_7D.2": "GRACE",
...
new_units_table = [
    [
        "{0}\n{1}".format(i[i.index(":") + 1 :], _UNITS_MAP[i][0]),
        "{0}".format(_UNITS_MAP[i][1]),
    ]
    for i in _UNITS_MAP
]
...
""".format(
    ...
    if len(words) == 2:
        ...
        variable = ":".join([_varmap[words[0]]] + words)
...
if __name__ == "__main__":
    ...
    r = ldas(
        ...
        variable="GLDAS:GLDAS_NOAH025_3H_v2.1.001:SOILM10-40cm",
        lon=100,
        lat=30,

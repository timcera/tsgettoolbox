...
def get_station_sensors(station_ids=None, sensor_ids=None, resolutions=None):
    ...
    for station_id in station_ids:
        ...
        var_resolution = [re.split(r'[\(\)]+', x)[1]
                          for x in sensor_list.resolution]

        sensor_list['resolution'] = var_resolution
        sensor_list['variable'] = [x + y
                                   for x, y in zip(var_names, var_resolution)]
        sensor_list['units'] = pd.Series(units,
                                         index=sensor_list.index)

        station_sensors[station_id] = _limit_sensor_list(sensor_list,
                                                         sensor_ids,
                                                         resolutions)
    ...
    for station_id, sensor_list in list(sensors.items()):
        ...
        for _, row in sensor_list.iterrows():
            ...
            url = 'http://cdec.water.ca.gov/cgi-progs/queryCSV' + \
                  ...
                  '&dur_code=' + res_to_dur_code_map[res] + \
                  '&sensor_num=' + str(sensor_id) + \
                  '&start_date=' + start_date_str + \
                  '&end_date=' + end_date_str
            station_data[var] = pd.read_csv(url,
                                            skiprows=2,
                                            header=None,
                                            parse_dates=[[0, 1]],
                                            index_col=None,
...
def _limit_sensor_list(sensor_list, sensor_ids, resolution):
    ...
    if sensor_ids is not None:
        sensor_list = sensor_list[[x in sensor_ids
                                   for x in sensor_list.sensor_id]]

    if resolution is not None:
        sensor_list = sensor_list[[x in resolution
                                   for x in sensor_list.resolution]]

    return sensor_list


res_to_dur_code_map = {'hourly': 'H',
                       'daily': 'D',
                       'monthly': 'M',
                       'event': 'E'}
